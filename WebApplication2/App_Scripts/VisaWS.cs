...
public class VisaWS : System.Web.Services.WebService {
    ...
    {
        string sqlStr = "Select [VisaNumber], [Cash], [VisaType], [AccountNumber], [Credit], [ExpirationDate] From [CreditCard]";
    ...
    public bool ValidVisaDetails(int visaNumber, int accNumber, string visaType, string date)
    {
        ...
        string sqlStr = "Select * From [CreditCard] where [VisaNumber]='" + visaNumber.ToString() + "' and [VisaType]='" + visaType.ToString() + "'and [AccountNumber]='" + accNumber.ToString() + "'";
        DataTable dt = DBFunctions.SelectFromTable(sqlStr, "UserDatabase.mdb");
        if (dt.Rows.Count>0)
    ...
    {double cash=0;
    int credit = 0;
        string sqlStr = "Select * From [CreditCard] where [VisaNumber]='" + visaNumber.ToString() + "' and [VisaType]='" + visaType + "'and [AccountNumber]='" + accountNumber.ToString() + "'";
        DataTable dt = DBFunctions.SelectFromTable(sqlStr, "UserDatabase.mdb");
        if (dt.Rows.Count > 0)
        {
    ...
    {double cash=0;
        ...
        cash = Convert.ToDouble(dt.Rows[0]["Cash"]);
        double result = cash - total;
        string SqlStr = "Update [CreditCard] Set [Cash]='" + result.ToString() + "' Where [VisaNumber]='" + visaNumber.ToString() + "' and [VisaType]='" + visaType + "'and [AccountNumber]='" + accountNumber.ToString() + "'";
    ...
    public void UpdateSentTo(string reciever, int quantity, double price)
    {
        ...
        string sqlStr = "Select * From [Customer] where [EmailAddress]='" + reciever + "'";
        DataTable dt = DBFunctions.SelectFromTable(sqlStr, "UserDatabase.mdb");
        if (dt.Rows.Count > 0)
        {
            recieverCash = Convert.ToDouble(dt.Rows[0]["Credit"].ToString());
            total += recieverCash;
            string SqlStr = "Update [Customer] Set [Credit]='" + total.ToString() + "' Where [EmailAddress]='" + reciever + "'";
            DBFunctions.ChangeTable(SqlStr, "UserDatabase.mdb");
        }
    }
}
